  ...
  knex.transaction((transaction) =>
    Promise.all([
      ...
      transaction.schema.withSchema("glacier").createTable("log_auth", (table) => {
        ...
        table.timestamp("timestamp", { useTz: true }).notNullable().defaultTo(knex.raw("CURRENT_TIMESTAMP"));
        table.integer("film_id").unsigned().notNullable().references("id").inTable("glacier.film");
        table.integer("key_id").unsigned().nullable().references("id").inTable("glacier.key");
