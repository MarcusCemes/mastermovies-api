...
export async function downloadThumbnail(
  ...
  try {
    ...
    const thumbnailResult = await getFilmThumbnail(
      req.app.db,
      req.params.film,
      req.params.thumbnail
    );
    if (!thumbnailResult) {
      next();
      return;
    ...
    const filePath = securePath(
      GlacierConfig.glacier_thumbnail_storage,
      req.params.thumbnail
    );
    if (!filePath) {
      next(
        new Error("Danger! Resolved path was outside of storage directory!")
      );
      return;
    }
    ...
    access(filePath, constants.F_OK, err => {
      ...
      if (err) {
        next(
          new Error(
            `Could not locate thumbnail for film ${req.params.film}: ${filePath}`
          )
        );
        return;
      }
