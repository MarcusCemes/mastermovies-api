 ...
 */
export async function downloadFilm(
  req: Request,
  res: Response,
  next: (err?: any) => void
): Promise<void> {
  ...
  try {
    const filmDownloadInfo = await getFilmDownloadInfo(
      req.app.db,
      req.params.film,
      req.params.export
    );
    ...
    if (typeof grant === "function") {
      ...
      const validAuthorization =
        ...
        token.glacier.authorizations[req.params.film] >=
          Math.round(Date.now() / 1000); // to seconds
...
function startDownload(
  ...
  if (!isValidHex(req.params.export)) {
    next(
      new Error(
        "Danger! Request parameter was unsafe: " +
          req.params.export.substring(0, 16)
      )
    );
    return;
  }
  ...
  const filePath = securePath(
    GlacierConfig.glacier_film_storage,
    req.params.export
  ); // has been sanitized
  ...
  access(filePath, constants.F_OK, err => {
    if (err) {
      next(
        new Error(
          `Could not locate file for export ${req.params.export}: ${filePath}`
        )
      );
      return;
    }
    ...
    try {
      ...
      if (typeof req.query.download === "undefined") {
        ...
        res.download(
          filePath,
          `${info.name} (${info.release.getFullYear()}).${extension}`
        );
      }
    } catch (err) {
      next(err);
